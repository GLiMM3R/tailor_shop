...
namespace app.Presentation.Report
{
    public partial class UserReportUC : UserControl
    {
        ...
        private async void UserReportUC_Load(object sender, EventArgs e)
        {
        ...
        {
            user_report_dgv.AutoGenerateColumns = false;
            user_report_dgv.Columns.Clear();

            user_report_dgv.Columns.AddRange(
                    ...
                    DataGridViewUtils.CreateTextBoxColumn(dataPropertyName: "TotalOrders", headerText: "ຈຳນວນລາຍການສັ່ງຊື້", dataGridViewContentAlignment: DataGridViewContentAlignment.MiddleCenter, autoSizeMode: DataGridViewAutoSizeColumnMode.Fill),
            ...
            user_report_dgv.CellFormatting += SaleReportDgv_CellFormatting;
        }

        private void SaleReportDgv_CellFormatting(object? sender, DataGridViewCellFormattingEventArgs e)
        {
            var columnName = user_report_dgv.Columns[e.ColumnIndex].DataPropertyName;
        ...
        public async Task LoadUserReport()
        {
            ...
            using (var dbContext = new AppDbContext())
            {
                ...
                var result = await report.GetUserReport(startOfDay, endOfDay, _pagination);
                _pagination.TotalItems = result.Total;
                user_report_dgv.DataSource = null;
                user_report_dgv.DataSource = result.Data.ToList();
                user_report_dgv.ClearSelection();
                UpdatePageNumber();
            }
            await LoadStatistic();
        }
        ...
        public async Task LoadStatistic()
        {
            ...
            using (var dbContext = new AppDbContext())
            {
                ...
                var result = await report.GetUserStatistic(startOfDay, endOfDay);

                if (result != null)
                {
                    total_users_lbl.Text = result.TotalOrders.ToString("N0");
                    total_orders_lbl.Text = result.TotalOrders.ToString("N0");
                    total_amount_lbl.Text = result.TotalAmount.ToString("N0");
                    aov_lbl.Text = result.AverageOrderValue.ToString("N0");
                }
            }
        }
        ...
        private async void export_btn_Click(object sender, EventArgs e)
        {
            using (var dbContext = new AppDbContext())
            {
                ...
                var result = await report.GetUserReport(startOfDay, endOfDay, allPagination);

                if (result.Data != null && result.Data.Any())
                {
                    var excelUtils = new ExcelUtils();
                    excelUtils.ExportToExcel(result.Data.ToList(), "User Report", "user_report_" + DateTime.Now.ToShortDateString().Replace("/", "-") + ".xlsx");
                }
                else
                {
